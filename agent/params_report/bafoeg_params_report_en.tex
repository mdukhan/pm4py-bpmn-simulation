...
\date{\today \\[1ex] \url{https://github.com/mdukhan/pm4py-bpmn-simulation}}
...
\subsubsection{From Abstract Business View to Database Logging}
The model now prioritizes the database and system log structure over an abstract business view. Activities correspond directly to database events (e.g., ``Application started'', ``Send Application Mail''), and subprocesses that do not generate distinct log entries have been removed. This ensures the model reflects the actual granularity of IT system event logs.

\subsubsection{Simplified BPMN Elements}
To ensure compatibility with Celonis, complex elements such as data objects, stores, and parallel gateways for data collection were removed or simplified. The focus remains on activity names, timestamps, and case IDs, avoiding the complexity of synchronization logic that standard event logs do not capture.

\subsubsection{Pool Reduction}
The model structure was reduced from four pools to two: \textbf{System} and \textbf{Clerk}. The \textbf{Student} and \textbf{Parents} pools were removed as external actors do not generate process-relevant log data. This focuses the analysis on internal processing performance and automation potential. 

\clearpage
\begin{sidewaysfigure}
    \centering
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{bafoeg_v1.png}
...
\begin{sidewaysfigure}
    \centering
    \includegraphics[width=\textwidth,height=\textheight,keepaspectratio]{bafoeg_ocel.png}
